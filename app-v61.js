...
let S={tab:get(LS.tab,'today'),days:normalizeDays(get(LS.days,D.trip),D.trip),journal:get(LS.journal,{}),expenses:get(LS.expenses,[]),checks:get(LS.checks,{}),girls:get(LS.girls,{greta:{},maria:{}}),girl:'greta',screen:null,selectedDay:'D0',more:'expenses',edit:null};
...
function normalizeDays(saved, defaults){
 if(!Array.isArray(saved)||!saved.length)return defaults;
 return defaults.map(def=>{
   const old=saved.find(x=>x&&x.id===def.id)||{};
   return {...def,...old,image:old.image||def.image,city:old.city||def.city,items:Array.isArray(old.items)&&old.items.length?old.items:def.items};
 });
}
...
function current(){
 ...
 if(now>b)return {mode:'after',day:S.days.at(-1)};
...
try{
 render();
 window.setTimeout(()=>window.__closeVW340Splash?.(),120);
}catch(error){
 console.error('VW340 v6.1 startup error',error);
 window.__closeVW340Splash?.();
 const host=document.getElementById('app');
 if(host)host.innerHTML='<main style="padding:32px;font-family:-apple-system,sans-serif"><h2>No se ha podido iniciar la app</h2><p>Cierra esta pestaña, vuelve a abrirla y recarga. La caché antigua ya se ha eliminado.</p></main>';
}
window.addEventListener('load',()=>window.setTimeout(()=>window.__closeVW340Splash?.(),180));
